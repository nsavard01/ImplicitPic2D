...
contains
    ...
    type(GSSolver) function GSSolver_constructor(omega, evenGridBool) result(self)
    ...
    subroutine constructPoissonOrthogonal(self, N_x, N_y, diffX, diffY, NESW_wallBoundaries, boundaryConditions)
        ...
        self%black_NESW_BoundIndx(6, self%numberBlackBoundNodes))
        ...
        if (self%evenGridBool) then
            ...
            allocate(self%matCoeffs(5,self%N_x,self%N_y))
        end if
        ...
        do j = 1, self%N_y
            do i = 1, self%N_x
                ...
                if (.not. self%evenGridBool) self%matCoeffs(:, i, j) = [C_O, C_N, C_E, C_S, C_W]
            end do
        end do

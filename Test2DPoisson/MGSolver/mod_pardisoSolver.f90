module mod_pardisoSolver
    ...
    type :: pardisoSolver
        ...
        real(real64), allocatable :: sourceTerm(:), MatValues(:), result(:)
...
contains
    ...
    type(pardisoSolver) function pardisoSolver_constructor(matDimension) result(self)
        ...
        self%iparm = 0
        self%pt = 0
        allocate(self%sourceTerm(matDimension), self%rowIndex(matDimension+1), self%result(matDimension))
    ...
    subroutine initializePardiso(self, mnum, mtype, maxfct, msglvl)
        ...
        call pardiso(self%pt, self%maxfct, self%mnum, self%mtype, 11, self%matDimension, self%MatValues, &
            self%rowIndex, self%columnIndex, 1, 1, self%iparm, self%msglvl, self%sourceTerm, self%result, self%error)
    ...
    subroutine runPardiso(self)
        ...
        ! phase 33, run and produce output to the result

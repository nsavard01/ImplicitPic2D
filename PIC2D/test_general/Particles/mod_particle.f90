...
contains
    ...
    type(Particle) function particle_constructor(mass, q, w_p, N_p, finalIdx, particleName, N_x, N_y) result(self)
        ...
        self%velocity(3,self%max_indx, number_threads_global), self%number_deletes_thread(number_threads_global))
        
        self%number_particles_thread = N_p/number_threads_global
        allocate(self%cell_count(N_x-1, N_y-1, number_threads_global))
        self%cell_count = 0
       
        allocate(self%cell_indx_array(N_x-1, N_y-1, number_threads_global))
    ...
    subroutine particle_mover_uniform(self, E_Field, world, del_t, count_bool)
        ...
        self%number_particles_thread(i_thread) = self%number_particles_thread(i_thread) - delete_idx
        self%number_deletes_thread(i_thread) = delete_idx
